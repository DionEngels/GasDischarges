...
PoissonVariable::PoissonVariable(const Grid &grid,
    ...
    : Field(grid.num_np()), lambda_cv(grid.num_ew(), 0.0),
      rho(grid.num_np(), 0.0), m_grid(grid), m_system(grid.num_np()),
      m_bc_w(bc_w), m_bc_e(bc_e), m_urf(1.0), m_flux_dens(grid.num_ew(), 0.0) {}

void PoissonVariable::DiscFluxDens(unsigned ndx_cv, double &DA,
                                   double &DB) const {
...
void PoissonVariable::AddVentzek(double dt, PhiVariable &nx, double qx) {
  ...
  for (unsigned i = 1; i != nx.grid().num_ew() - 1; ++i) {
    ...
    const double cf =
        qdt_dx * nx.grid().area_ew(i) * mumid * (nx[iW] * g1 - nx[iE] * g2);
    system().ae(iW) += cf;
    system().ap(iW) += cf;
    system().aw(iE) += cf;
    system().ap(iE) += cf;
    const double src =
        qdt_dx * nx.grid().area_ew(i) * Dmid * (nx[iE] - nx[iW]) * h;
    system().b(iW) -= src;
    system().b(iE) += src;
  }
};