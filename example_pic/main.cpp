...
const double simtime = 1e-5;		// seconds, complete simulation time.
...
// this file.
void WritePoisson(const Grid& grid, const Field& poisson,
		const Field& rho_e, const Field& rho_i,
		std::ostream& os);
...
int main()
{
	...
	{	
		...
		if (e_sum > e_max_crosssec) e_max_crosssec = e_sum;
		// ions:
		const double i_sum = i_chargeexchange.CrossSec(energy) +
				i_elastic.CrossSec(energy);
		if (i_sum > i_max_crosssec) i_max_crosssec = i_sum;
	}
	...
	for (unsigned p=0;p<initial_e_nr;++p)
	{
		...
		speed[0] = rnd(); speed[1] = rnd(); speed[2] = rnd();
		speed *= std::sqrt(2.0*eps/electrons.Mass());
		const double dtcol = -(1.0/(N*e_max_crosssec*abs(speed)))*
							std::log(rnd());
	...
	for (unsigned p=0;p<initial_i_nr;++p)
	{
		...
		const double dtcol = -(1.0/(N*i_max_crosssec*abs(speed)))*
							std::log(rnd());
	...
	for (unsigned long ti=0;ti<simtime/dt;++ti)
	{
		...
		if (!(ti%nrout)) 
		{
			...
			time_str << t;
			//plot
			grid.plot(poisson, rho_e + rho_i, "Position (cm)", "Potential (V)", "Charge density (C m^-3)", "time = " + time_str.str() + " s");
		}
		...
		for (unsigned p=0; p<nrelectrons;++p)
		{
			...
			//    position:  
			const double E = CalcEfield(grid,poisson,
						electrons[p].Position());
			...
			if (electrons[p].Active() && (electrons[p].Tcol()<=t)) 
			{
				...
				{
					sum += e_proclist[i]->
						CrossSec(electrons[p].Energy());
					if (rndsc > sum) process++;
				}
				...
				double dtcol = -(1.0/(N*e_max_crosssec*vabs))*
						std::log(1.0-rnd());
		...
		for (unsigned p=0; p<nrions;++p)
		{
			...
			if (ions[p].Active() && (ions[p].Tcol() <= t)) 
			{
				...
				double dtcol = -(1.0/(N*i_max_crosssec*vabs))*
						std::log(1.0-rnd());
				ions[p].Tcol() += dtcol;
			}
		}
		...
		if (Vdischarge<0.0) Vdischarge=0.0;
		// set the voltage.
		poisson_left.Set(-Vdischarge);

		// some diagnostics:
		const double ne_average = 
				electrons.size()*electrons.Weight()/gridsize;
		...
		const double v_mean = std::sqrt(2.0*electrons.MeanEnergy()/
									PhysConst::me);
		const double nsigmav = 	N*e_max_crosssec*v_mean;

		// write some data:
		if (!(ti%nrout)) std::cerr << ti << '\t' << t << '\t' 
			<< electrons.size() << '\t' 
			<< ions.size() << '\t'
			<< I << '\t'
			<< Vdischarge << '\t'
...
double Ambi_Diff_Coeff(double Te, double Th)
{
	...
	// thermal Velocity
	const double vth=std::sqrt(PhysConst::pi*Argon::Mass()*Th/(8.0*PhysConst::k_b));
