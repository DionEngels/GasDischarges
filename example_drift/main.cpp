...
//              c:\npf-practicum> example_drift | c:\gnuplot\binaries\pgnuplot.exe
...
int main()
{
	...
	std::cout << "set yrange [" << particleweight * 25 << ":" << initial_e_nr * particleweight * 25 << "]" << std::endl;
	...
	for (unsigned long ti=0;ti<simtime/dt;++ti)
	{
		...
		if (!(ti%nrout)) {
		 std::cerr << t << '\t' 
			...
			grid.plot(inv_charge, "Position (cm)", "Particle density (m^-3)", "Time = " + time_str.str() + " s");
		}
