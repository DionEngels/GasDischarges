...
class Grid
{
  public:
	 ...
	 *  points of this grid.
	 */
	void plot(const Field& f, std::string xlabel="", std::string ylabel="", std::string title="") const;
	 ...
	 */
	void plot(const Field& f1, const Field& f2, std::string xlabel="", std::string ylabel="", std::string y2label="", std::string title="") const;
  ...
  private:
	const GridType m_grid_type; ///< type of the grid (Cartesian, Cylindrical, Spherical)
	...
	/// returns the grid type-dependent perpendicular area [m^2] at coordinate value \a c.
