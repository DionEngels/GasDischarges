...
double tolerance=5e-2;		// the allowed relative change of a variables after a time step
const unsigned iter_log = 250;// number of iterations after which info is written to screen
...
const double dt_out = period/(NoutPerCycle*freq);	// [s], between writing output.
...
int main()
{
	...
	while (t<simtime)
	{
		...
		for (unsigned i=0; i<grid.num_np(); ++i)
		{
			...
			{
				const double ge = 0.5*(dens_e.flux_density()[i-1] + dens_e.flux_density()[i]);
		...
		dens_i.DiscFluxDens(ndx_e_cv,dens_i.system().ap(ndx_e_np), dens_i.system().aw(ndx_e_np));
		dens_i.system().ap(ndx_e_np) +=Vi_e;

		dens_e.DiscFluxDens(ndx_e_cv,dens_e.system().ap(ndx_e_np), dens_e.system().aw(ndx_e_np));
		...
		{
			J_cv[i]=PhysConst::e*(
					+dens_i.flux_density()[i]
					-dens_e.flux_density()[i]);
		}	
