...
void Swarm::WriteEDF(std::ostream &os) const {
  ...
  for (unsigned i = 0; i < eedf.size(); ++i) {
    os << eedf.Energy()[i] / PhysConst::e << '\t' << eedf.Value()[i]
       << std::endl;
  }
...
Field Swarm::MeanEnergyField(const Grid &grid) const {
  ...
  for (unsigned p = 0; p < size(); ++p) {
    ...
    //     nearest grid cells.
    energy[j] +=
        (grid.pos_np(j + 1) - (*this)[p].Position()) * (*this)[p].Energy() / dx;
    nrincell[j] += (grid.pos_np(j + 1) - (*this)[p].Position()) / dx;
    energy[j + 1] +=
        ((*this)[p].Position() - grid.pos_np(j)) * (*this)[p].Energy() / dx;
    nrincell[j + 1] += ((*this)[p].Position() - grid.pos_np(j)) / dx;
  }
  // because of the specifics of the grid (nodal point 0 and nrpoints+1
  // are really fake points) we need to do a little correction.
  energy[0] = 0.0;
  energy[grid.num_np() - 1] = 0.0;
...
void Swarm::WriteMeanEnergyField(const Grid &grid, std::ostream &os) const {
  Field energies = MeanEnergyField(grid);
  for (unsigned i = 0; i < grid.num_np(); i++) {
    os << i << '\t' << grid.pos_np(i) << '\t' << energies[i] / PhysConst::eV
       << std::endl;
  }
  os << std::endl << std::endl;
}
...
void HardSphereProcess::DoCollide(Particle &p1, Particle &p2) const {
  ...
  const npfGeomVector u_M =
      (p1.Mass() * p1.Velocity() + p2.Mass() * p2.Velocity()) / mtotal;
  ...
  u_r *= sqrt(1.0 - m_delta_eps * (2.0 * mtotal / (p1.Mass() * p2.Mass())) /
                        u_r_abssqr);
...
void ProcessList::AddProcess(Process *prcss) { push_back(prcss); }

void ChargeExchangeProcess::DoCollide(Particle &p1, Particle &p2) const {
  // Do a little check:
